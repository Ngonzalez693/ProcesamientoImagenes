%% Interfaz Gráfica
function interfaz_image_processing

    % Constantes y parámetros de configuración
    CONFIG.DEFAULT_AREA_MIN = 800;
    CONFIG.DEFAULT_COLOR = [1 0 0]; % Rojo
    shapes = {'Contorno', 'Rectángulo', 'Círculo', 'Todos'};
    CONFIG.DEFAULT_SHAPE = 'Todos';
    CONFIG.DEFAULT_MORPHOLOGIC_OPERATION = 'Cierre';
    CONFIG.DEFAULT_ELEMENT_SIZE = 1;

    % Variables
    img = [];
    imgGray = []; % Imagen en escala de grises
    bwBase = [0 0 0]; % Imagen binarizada
    ...
    modoColor = 'text'; % o 'detection'

    state = struct();
    state.areaMin = CONFIG.DEFAULT_AREA_MIN;
    state.color = CONFIG.DEFAULT_COLOR;
    state.shape = CONFIG.DEFAULT_SHAPE;
    state.opMorphologic = CONFIG.DEFAULT_MORPHOLOGIC_OPERATION;
    state.elementSize = CONFIG.DEFAULT_ELEMENT_SIZE;

    % Buscar índice del valor por defecto
    defaultIndex = find(strcmp(shapes, state.shape));
    ...
    % SLIDER Área Mínima
    uicontrol('Style', 'text', 'String', 'Área mínima (px):', ...
            'Position', [85 495 100 20]);
    sliderArea = uicontrol('Style', 'slider', 'Min', 100, 'Max', 10000, ...
                         'Value', state.areaMin, ...
                         'Position', [85 475 100 20], ...
                         'Callback', @actualizarDetection);
    areaValueLabel = uicontrol('Style', 'text', 'String', num2str(state.areaMin), ...
                            'Position', [190 472 30 20], ...
                            'Callback', @actualizarDetection);

    % Forma del contorno
    uicontrol("Style", "text", "String", 'Forma de contorno:', ...
            'Position',[85 445 100 20]);
    shapeDrop = uicontrol("Style", "popupmenu", ...
                        "String", shapes, ...
                        'Value', defaultIndex, ...
                        "Position", [85 425 100 20], ...
                        "Callback", @actualizarDetection);

    % Conteo de objetos
    countLabel = uicontrol('Style', 'text', 'String', 'Elementos Detectados: 0', ...
                        'Position', [85 390 100 30]);

    % SLIDER umbral 1
    uicontrol('Style', 'text', 'String', 'Umbral 1', 'Position', [85 330 100 20]);
    umbral1Slider = uicontrol('Style', 'slider', 'Min', 0, 'Max', 255, 'Value', 0, ...
        'Position', [85 310 100 20], 'Callback', @actualizarUmbral1);
    umbral1Text = uicontrol('Style', 'text', 'String', '0', 'Position', [190 307 30 20]);

    % SLIDER umbral 2
    uicontrol('Style', 'text', 'String', 'Umbral 2', 'Position', [85 280 100 20]);
    umbral2Slider = uicontrol('Style', 'slider', 'Min', 0, 'Max', 255, 'Value', 255, ...
        'Position', [85 260 100 20], 'Callback', @actualizarUmbral2);
    umbral2Text = uicontrol('Style', 'text', 'String', '255', 'Position', [190 257 30 20]);

    % TEXTO de mensaje de validación
    mensajeText = uicontrol('Style', 'text', 'String', '', ...
        'FontSize', 9, 'ForegroundColor', 'red', ...
        'Position', [80 215 130 40], 'HorizontalAlignment', 'center');
    
    % MENÚ DESPLEGABLE
    uicontrol('Style', 'text', 'String', 'Seleccionar Operador:', 'Position', [85 190 100 25]);
    ...
    %actualizarDetection(); % Ejecución Inicial
    ...
    function cargarImagen(~, ~)
        ...
        img = imread(fullfile(path, file)); % Imagen Original
        imgGray = rgb2gray(img); % Convertir a escala de grises
        bwBase = imbinarize(imgGray); % Binarizar
    ...
    function actualizarTamanoFuente(~, ~)
     ...
     %% Funciones para los sliders de color
    ...
    function actualizarColorR(~, ~)
        ...
        if strcmp(modoColor, 'detection')
            % Cambia el color del contorno de la detección
            state.color = [colorTextoR, colorTextoG, colorTextoB] / 255;
            actualizarDetection();
        end
    end
    ...
    function actualizarDetection(~, ~)

        modoColor = 'detection';

        % Leer estado actual
        state.areaMin = round(sliderArea.Value);
        state.color = [colorRSlider.Value, colorGSlider.Value, colorBSlider.Value] / 255;
        val = get(shapeDrop, 'Value'); % Obtener el valor seleccionado del popup menu
        shapes = get(shapeDrop, 'String');
        state.shape = shapes{val};
        
        % Actualizar etiquetas con valores actuales
        set(areaValueLabel, 'String', num2str(state.areaMin));
        
        % Procesamiento mejorado de la imagen
        % Invertir imagen si es necesario (para objetos claros en fondo oscuro)
        bw = bwBase;
        bw = imclose(bw, strel('disk', state.elementSize));
        
        % Filtrado por área mínima más agresivo
        bw = bwareaopen(bw, state.areaMin);
        ...
        imshow(label2rgb(L, @jet, [.5 .5 .5]), 'Parent', ax1);
        hold(ax1, 'on');
        ...
        for k = 1:length(B)
            ...
            switch state.shape
                case 'Contorno'
                    plot(ax1, boundary(:,2), boundary(:,1), 'Color', state.color, 'LineWidth', 2);
                case 'Rectángulo'
                    rectangle(ax1, 'Position', box, 'EdgeColor', state.color, 'LineWidth', 2);
                case 'Círculo'
                    viscircles(ax1, centroid, radius, 'Color', state.color, 'LineWidth', 1.5);
                case 'Todos'
                    plot(ax1, boundary(:,2), boundary(:,1), 'Color', state.color, 'LineWidth', 1.5);
                    rectangle(ax1, 'Position', box, 'EdgeColor', state.color, 'LineWidth', 1);
                    viscircles(ax1, centroid, radius, 'Color', state.color, 'LineWidth', 0.8);
                    disp('Contorno funcionando');
            end
        end
        ...
        set(countLabel, 'String', ['Objetos detectados: ', num2str(length(B))]);
